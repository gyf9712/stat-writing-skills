...
- Body-prose parentheses: prohibited except for citations, equation numbers, formal cross-references (section/appendix/theorem/figure/table/supplement), assumption/step labels, acronym first-use in either order, units and measurement scales, short technical qualifiers (`(in probability)`, `(up to constants)`, `(say, $c=1/2$)`), and short mathematical annotations. Restructure or cut every other parenthetical.
...
- Paragraph-level formatting: no run-in `\paragraph{}` / `\subparagraph{}` headings as rhetorical labels; they are acceptable as genuine low-level headings, proof-object headers, or semantic stage labels in a long proof, where they must name the object or stage rather than sell the result. Otherwise use a real subsection when a break is warranted, or a natural prose transition. Do not fragment exposition into short one-idea paragraphs as a substitute for connective writing.
