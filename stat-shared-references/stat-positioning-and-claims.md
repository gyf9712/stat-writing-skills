...
The audit is well-suited to a Codex MCP call because Codex (GPT-5.6 xhigh) can independently bring positioning and claim-strength judgments. The call is a dialogue, not a directive. Read `stat-codex-dialogue.md` before starting. The initial Codex review opens the conversation; the value comes from pushback and convergence using `mcp__codex__codex-reply`.
